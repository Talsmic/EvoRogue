///window_spreadsheet_draw(mode,x,y);
/// @arg mode
/// @arg x
/// @arg y
var mode = argument0;
...
for ( var i=1 ; i<rowcount ; i+=2 ) {
	draw_colouredplane(X+2,Y+i*11+12,width-4,11,c_midgray,0.3);
	};	
//Draw Columns
var XX = X+2
for ( var i=1 ; i<array_length_1d(col_width) ; i+=2 ) {
	XX += col_width[i-1];
	draw_colouredplane(XX,Y+23,col_width[i],height-15,c_white,0.05);
	XX += col_width[i];
	};	
//Draw Headers
