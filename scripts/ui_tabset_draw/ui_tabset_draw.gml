...
for ( var i=1 ; i<array_length_2d(Tabset_List,ref) ; i++ ) {
	...
	//State Detection
	var state = eIconState.enabled
	if ( infocus() and mouse_in_region_gui( define_region( X-1-abs(x1), Y-tabheight-2, tabwidth+abs(x1), tabheight+abs(y1) ) ) ) {
		if ( mouse_check_button(mb_left) ) {	var state = eIconState.pressed;	}
			else { var state = eIconState.mouseover; };
		};
	if ( !Tabset_TabEnabled[ref,i] ) { var state = eIconState.disabled }; 
	if ( Tabset_Pointer[ref] == i ) { state = eIconState.focus; };
